...
class optoStim(SimulatorMod):
    def __init__(self, node_set, section_name, section_index=0, section_dist=0.5, stim_params=None, pulse_file=None,
                 output_file="opto_stim_currents.h5", tmp_dir="./tmp", intensity_file_path=None, probe_file_path=None, uptake_prob=1.,
                 record_data=True,combine_probes=True):
        """
        ...
        # allow overriding recording behavior from stim params (e.g., in opto_input section)
        # use key 'record_currents' to control whether currents are recorded to disk
        if 'record_currents' in self._stim_params:
            self.record_data = bool(self._stim_params.get('record_currents'))
        else:
        ...
        self.intensity_file_path = intensity_file_path
        self.probe_file_path = probe_file_path
        self.uptake_prob = uptake_prob
        self.combine_probes = combine_probes
        ...
        if MPI_RANK == 0 and self.record_data:
            if os.path.isfile(self._output_file):
                os.remove(self._output_file)
        pc.barrier()  # wait up guys dont leave without me

        # Load light scattering data (raise if missing path)
        if self.intensity_file_path is not None:
            self.rad_dist, self.depth_dist, self.intensity_dist = self._get_light_scattering(self.intensity_file_path)
        else:
            self.rad_dist, self.depth_dist, self.intensity_dist = (None, None, None)
    ...
    def _create_tmp_file(self, nsteps, nsites, nprobes):
        ...
        if self.combine_probes:
            self._tmp_handle.create_dataset(
                '/stim/data', (nsteps, nsites, 1), dtype=np.float64, chunks=True, maxshape=(None, nsites, 1))
        else:
            self._tmp_handle.create_dataset(
    ...
    def _write_block_data(self, block_interval):
        ...
        if not self.record_data:
            return
        if self.combine_probes:
            self._tmp_handle['/stim/data'][start_step:end_step, :, 0] = np.sum(self._block_data[start_step:end_step, :, :], axis=-1)
        else:
            ...
            with h5py.File(self._output_file, 'w') as out_file:
                ...
                if self.combine_probes:
                                out_file.create_dataset(
                                    '/stim/data', (nsteps, total_nsites, 1), dtype=np.float64, chunks=True)
                else:
                    out_file.create_dataset(
    ...
    def initialize(self, sim):
        ...
        io.log_info(f"Max light intensity set to {max_intensity}")
        ...
        # Initialize block data array and temporary file to store current values only if recording enabled
        if self.record_data:
            if self.combine_probes:
                self._create_tmp_file(nsteps, nsites, 1)
                self._block_data = np.zeros((nsteps, nsites, 1))
            else:
                self._create_tmp_file(nsteps, nsites, nprobes)
                self._block_data = np.zeros((nsteps, nsites, nprobes))
        else:
            # not recording, set placeholders
            self._block_data = None
        ...
        pc.barrier() # make sure the stim it set up before moving forward
        total_intensity_count = pc.allreduce(self.intensity_count, 1)
        io.log_info(f"Cells and probe combos with light intensity above 0.01 = {total_intensity_count}")
    ...
    def step(self, sim, tstep):
        ...
        for gid, vec in self.currents.items():
            ...
            for j, probe in enumerate(vec):
                ...
                if self.combine_probes:
                    self._block_data[tstep, local_idx, 0] = sum(probe.x[-1] for probe in vec)
                else:
    ...
    def finalize(self, sim):
        ...
        # Close the temporary file handle if it was created
        ...
        pc.barrier()
        if self.record_data == False:
            # Clean up any temporary files on rank 0 (if present) and return
            if MPI_RANK == 0:
                for rank in range(N_HOSTS):
                    tmp_file = self._get_tmp_fname(rank)
    ...
    def _get_light_scattering(self,npy_filepath):
        """Reads in the light scattering data from the monte carlo simulation

        Args:
            npy_filepath(str): string to the constructed numpy file which has the info for how light scatters

        """
        loaded_data = np.load(npy_filepath, allow_pickle=True).item()
        R_loaded = loaded_data['R']
        Z_loaded = loaded_data['Z']
        Intensity_loaded = loaded_data['Intensity']
        # Make sure intensity doesn't go below 0
        Intensity = np.maximum(Intensity_loaded, 0)

        # Normalize the intensity (scale between 0 and 1)
        Intensity = Intensity / np.max(Intensity)

        return R_loaded,Z_loaded,Intensity
    ...
    def _get_intensity(self,cell,R,Depth,plot_mat,probe_position=[0,0,0]):
        ...
        if np.any(np.isnan(node_intensities)):
            io.log_warning(f"Warning: NaN values detected in node intensities at indices {np.isnan(node_intensities)}")
    ...
    def _create_stim(self, hobj_sec, cell,probe_pos,gid,probe_number):
        ...
        max_intensity = self._stim_params.get("light_intensity")
        norm_intensity = self._get_intensity(probe_position=probe_pos,cell=cell,R=self.rad_dist,Depth=self.depth_dist,plot_mat=self.intensity_dist)
        light_intensity = max_intensity*norm_intensity 
        # possible speed up if the intensity is 0 why insert the channel? lets try it
        if light_intensity > 0.0001: # maybe setting to zero would be bad so if the intensity is this small probs wont contribute. Setting to zero COULD be ok too not sure
            ...
            # Only create and record NEURON Vectors if recording is enabled
            if self.record_data:
