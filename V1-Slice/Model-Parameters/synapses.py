...
def lognormal_weight(edge_props, source, target):
    ...
    is_gap = get_edge_prop("is_gap_junction")
    # gap juctions are different in how they are set up compared to chem syn
    # since you have to assign a syn_weight for the gap junction i have this different way to override the set weight
    if is_gap:
        syn_data = get_edge_prop("dynamics_params")
        mean = syn_data['initW']
        stdev = mean*0.3
...
GABA_A_STP_params = ('tau_r_GABAA', 'tau_d_GABAA', 'e_GABAA', 'Use', 'Dep', 'Fac')

def GABA_A_STP(syn_params, sec_x, sec_id):
...
def gaba_a_stp(syn_params, xs, secs):
    """Create a list of GABA_A_STP synapses
    :param syn_params: parameters of a synapse
    :param xs: list of normalized distances along the section
    :param secs: target sections
    :return: list of NEURON synpase objects
    """
    return [GABA_A_STP(syn_params, x, sec) for x, sec in zip(xs, secs)]
...
def load(randseed=1111, rng_obj=None):
    ...
    add_synapse_model(GABA_A_STP, 'GABA_A_STP', overwrite=False)
    add_synapse_model(GABA_A_STP, overwrite=False)
